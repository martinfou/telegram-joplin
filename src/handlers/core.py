...
def _build_greeting_response(user_id: int, orch: TelegramOrchestrator) -> str:
    ...
    return (
        ...
        "<b>💪 Health</b>\n"
        "• /health_import — Then send .csv (no caption needed), or caption /health_import on the file; "
...
def _helpme(orch: TelegramOrchestrator):
    async def handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        help_message = (
            ...
            "🔹 /health_import — Run command, then send .csv (no caption) OR attach file with caption /health_import; "
