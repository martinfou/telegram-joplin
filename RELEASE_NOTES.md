...
### New Features
- **URL preview images from Gemini** — Notes created from links now get a preview image generated by Gemini from page metadata (title, description, content type) instead of a PageShot screenshot. Recipe links use an appetizing food-photo prompt; other links use a clean illustration prompt. No external screenshot service or API key needed beyond `GEMINI_API_KEY`.
- **Task duplicate detection with note index** — Duplicate-task check before adding a Google Task can now use the note index (semantic search) when available, so the bot can suggest existing notes that match the task title. `detect_duplicate_task` is async and accepts an optional `note_index` parameter.

### Internal
- Removed `src/url_screenshot.py` (PageShot); added `src/url_image.py` for Gemini-based URL preview images.
- Updated [docs/api-reference.md](docs/api-reference.md): PageShot section removed; URL preview images documented under Gemini.
- Mypy: typed project report `projects` list in `core.py` to fix inference errors.
