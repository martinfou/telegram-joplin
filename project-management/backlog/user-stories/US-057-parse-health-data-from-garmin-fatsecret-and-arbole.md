...
- 2026-04-06 - **Partial delivery** on `main`: import + summaries + DB + fixtures; **`/health_goal`** / **`/health_goals`** and rolling 7-day adherence on **`/health_week`**; **weekly Joplin note** after import (`YYYY-Www - Health Summary` under `01 - Areas / 💪 Health & Fitness / Weekly Health`, ISO Mon–Sun rollups + goals). **Screenshot OCR** after `/health_import` or caption `/health_import` on photo (Gemini, same pipeline as photo capture). Still open: preview/confirm wizard, AI insights (AC-8), trust/diff UX (AC-7).
